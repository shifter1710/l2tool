...
кликабельные колонки для профиля «Запись»: `Zapis`, `SIP stack` и `BFF`.
...
направление звонка сохраняется только как диагностические метаданные. Звонки,
которые старше 5 суток (120 часов) относительно текущего времени UTC, полностью
исключаются из результата. Строки с некорректными номерами или временем
сохраняются без ссылок и перечисляются в предупреждениях CLI.
