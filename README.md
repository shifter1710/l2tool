...
По умолчанию рядом появится `выгрузка.cleaned.xlsx`. Можно явно указать файл,
лист и окно ссылки относительно старта звонка:
...
`SIP stack prod` время из таблицы переводится из UTC в московское и задаётся
окно ±60 минут. Ссылка `MGW` ищет номер абонента в логах `mgw.service` в том же
временном окне. Звонки, которые старше 5 суток (120 часов) относительно текущего
