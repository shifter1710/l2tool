...
В очищенной таблице остаются только пять перечисленных столбцов и кликабельная
колонка `Ссылка` на настроенный дашборд `services.zapis`. Зелёным отмечается
инициатор звонка, голубым — принимающая сторона: для `out` пользователь является
инициатором, для `in` — принимающей стороной. Строки с некорректными номерами или
временем сохраняются без ссылки и перечисляются в предупреждениях CLI.
