...
## Синтетические данные в примерах и тестах

- В тестах, примерах (`examples/`), плейсхолдерах форм и документации —
  только синтетические данные. Реальные номера абонентов, сервисные номера,
  короткие коды и даты из производственных заявок не попадают в репозиторий
  ни в каком виде — даже «для примера».
- Условные диапазоны: клиенты — `79991234567`, абоненты — `79991000001…`,
  сервисные номера — `79991230001…`, короткие коды — `0900`. Даты в фикстурах
  произвольные и не привязаны к реальным обращениям.
- Реальные выгрузки и заявки живут только в игнорируемых Git путях:
  `tickets/`, `history/`, `cases/`, `*.parsed.json`, `parser_issues/`,
  `diagnostic_sources.json`, `backups/`.
...
3. Убедиться, что локальные данные и секреты не попали в Git, а примеры и
   тесты содержат только синтетические номера (см. раздел
   «Синтетические данные в примерах и тестах»).
