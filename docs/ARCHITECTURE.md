...
```mermaid
flowchart TB
    input["Выгрузка истории баланса"] --> which{"Формат записей?"}
    which -->|"сводный: дата + диапазон (длительность)<br/>и строка «А → Б» / «А → Б ← В (условие)»"| range["Сводный парсер:<br/>А — звонящий, при «← В» звонок<br/>приходит клиенту В по переадресации;<br/>направление «А → Б» — по msisdn,<br/>без него клиент = самый частый участник"]
    which -->|"событийный (прежний скрипт):<br/>заголовок события, стрелка, Duration"| parse["Построчный парсер:<br/>направление по стрелке,<br/>короткие коды (0900) как есть"]
    ...
    range --> ctx["call_context: msisdn + А/Б + время, tz МСК"]
    merge --> ctx
    single --> ctx
